...
from espnet2.enh.layers.mask_estimator_ext import MaskEstimatorExt
...
class DNN_Beamformer_Extractor(torch.nn.Module):
    ...
    def predict_mask(
        ...
        input_aux: Union[torch.Tensor, ComplexTensor], ilens_aux: torch.Tensor,
        ...
        Args:
            ...
            ilens (torch.Tensor): (B,)
            input_aux (torch.complex64/ComplexTensor): (B, T, F)
            ilens_aux (torch.Tensor): (B, )
        Returns:
            masks (torch.Tensor): (B, T, C, F)
            ilens (torch.Tensor): (B,)
        """

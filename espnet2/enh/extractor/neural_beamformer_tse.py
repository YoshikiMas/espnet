...
from espnet2.enh.layers.dnn_beamformer_ext import DNN_Beamformer_Extractor
from espnet2.enh.layers.dnn_wpe import DNN_WPE
from espnet2.enh.separator.abs_extractor import AbsExtractor


class NeuralBeamformerExtractor(AbsExtractor):
    def __init__(
        self,
        input_dim: int,
        num_spk: int = 1,
        ...
        assert num_spk == 1, "TSE beamformer only supports the single target speaker"
        ...
        if self.use_wpe:
            ...
            # WPE part is kept to be independent of the given embedding
            self.wpe = DNN_WPE(
                wtype=wnet_type,
                widim=input_dim,
                wlayers=wlayers,
                ...
                nmask=1,  # Number of speaker extracted is 1
        ...
        if self.use_beamformer:
            self.beamformer = DNN_Beamformer_Extractor(
                bidim=input_dim,
                btype=bnet_type,
                blayers=blayers,
                bunits=bunits,
    ...
    def forward(
        ...
        ilens: torch.Tensor,
        input_aux: Union[torch.Tensor, ComplexTensor],
        ...
        if (
            ...
            if self.use_beamformer and input.dim() == 4:
                others_b, ilens = self.beamformer.predict_mask(input, ilens, input_aux, ilens_aux)
                for spk in range(self.num_spk):

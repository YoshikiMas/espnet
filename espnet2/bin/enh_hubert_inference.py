...
def inference(
    ...
    for i in range(separate_speech.num_spk):
        ...
        writers.append(NpyAppendArray(f"{output_dir}/features/enhanced_{i}_{separate_speech.num_spk}.npy"))
        length_writers.append(open(f"{output_dir}/features/enhanced_{i}_{separate_speech.num_spk}.len", "w"))
    ...
    for keys, batch in loader:
        ...
        for (spk, w) in enumerate(waves):
            for b in range(batch_size):
                writers[spk].append(w[b])
                length_writers[spk].write(f"{len(w[b])}\n")

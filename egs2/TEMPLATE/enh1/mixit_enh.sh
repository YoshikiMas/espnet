...
inf_num=4   # Number of inferences
noise_type_num=1
dereverb_ref_num=1
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        # tempdir=$(mktemp -d "/tmp/xkc09_chime4-$$.XXXXXXXX")
        tempdir="/scratch/xuankaic/data_wsj02mix_libri2mix"

        # trap 'rm -rf ${tempdir}' EXIT

        # mkdir -p ${tempdir}
        # rsync -z -av --progress "${data_feats}" ${tempdir}
...
if ! "${skip_eval}"; then
    ...
    if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            # shellcheck disable=SC2086
            ${_cmd} JOB=1:"${_nj}" "${_logdir}"/mixit_enh_scoring.JOB.log \
                ${python} -m espnet2.bin.mixit_enh_scoring \
                    --key_file "${_logdir}"/keys.JOB.scp \
                    --output_dir "${_logdir}"/output.JOB \
                    ${_ref_scp} \
                    ${_inf_scp} \
        ...
        log "Evaluation result: ${enh_exp}/RESULTS.md"
